...
const SpawnProcess = /** @class */ (function () {
  function SpawnProcess() {
  }
  SpawnProcess.pathExists = function (path) {
    try {
  ...
  /**
     ...
     */
  SpawnProcess.spawn = function (command, args, path, options) {
    let customEnv = {};
    if (options && options.env) {
      customEnv = options.env;
    }
    const spawnedProcess = child_process_1.spawn(command, args);
  ...
  /**
     ...
     * information.
     */
  SpawnProcess.exec = function (command, options) {
    return new Promise(((resolve, reject) => {
      ...
      const spawnedProcess = child_process_1.execFile(command, execOptions, (err, stdout, stderr) => {
        if (!err) {
