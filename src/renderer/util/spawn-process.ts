 ...
 */
export interface ISpawnExecutionOptions {
  /**
 ...
 */
export interface IProcessExecutionOptions {
  /**
   * An optional collection of key-value pairs which will be
   * set as environment variables before executing the git
   * process.
...
export class SpawnProcess {
   ...
   */
  public static spawn(command: string,
                      args: string[],
                      path: string,
                      options?: ISpawnExecutionOptions): ChildProcess {
    let customEnv = {}
  ...
  public static exec(command: string,
                     options?: IProcessExecutionOptions): Promise<IProcessResult> {
    return new Promise<IProcessResult>(function (resolve, reject) {
      let customEnv = {}
      if (options && options.env) {
        customEnv = options.env
      }
      ...
      const spawnedProcess = execFile(command, execOptions, function (err: Error | null,
                                                                                stdout,
                                                                                stderr) {
        if (!err) {
